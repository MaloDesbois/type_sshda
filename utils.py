...
def selection(data,nbr,role): # role peut être "source" ou "target"
    ...
    if role == "source":
        ...
        for label in range(11):
            ...
            if len(indices)>nbr : 
                ...
                indices_ul1 = indices[:len(indices)//2]
        ...
        return selection_finale_ul1, selection_finale_ul2 # attention ici les données sont triées par classe
...
def data_loading(datas ,nbr_s=100000,nbr_t=100000): #data est de la forme [[jeux_sources],[jeux_targets]]
        ...
        for data in datas[1] : # les données target sont ajoutées
            ...
            data_train1, data_train2 = selection(data_train, nbr_t, role="target")
